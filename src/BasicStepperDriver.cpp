...
void BasicStepperDriver::startMove(long steps)
{
    ...
    if (steps_remaining){
        ...
        // set up new move
        if(steps >= 0) directionCCW = false;
        else directionCCW = true;

        //invert count direction!
        if(invertDirection)
        {
          //Set output pin
          if(directionCCW) digitalWrite(dir_pin, HIGH);
          else if (!directionCCW) digitalWrite(dir_pin, LOW);
        }
        else
        {
          //Set output pin
          if(directionCCW) digitalWrite(dir_pin, LOW);
          else if (!directionCCW) digitalWrite(dir_pin, HIGH);
        }
        ...
        case LINEAR_SPEED:
            ...
            step_pulse = timerFreq*0.676*sqrt(2.0f/(profile.accel*microsteps));
            //step_pulse = timerFreq /  sqrt((2*profile.accel));
            break;
...
long BasicStepperDriver::nextAction(void)
{
    ...
    if (steps_remaining > 0)
    {
        ...
        {
          TIM14->CR1 &= (~((uint16_t)TIM_CR1_CEN));
          //ms to wait if we have 1kHz ticks
        ticksToWait = next_action_interval;
...
//        };
        ...
        //Count the actual steps!
        if(directionCCW) actualPositionInSteps--;
        else actualPositionInSteps++;

        // account for calcStepPulse() execution time; sets ceiling for max rpm on slower MCUs
        //last_action_end = micros();
        next_action_interval = pulse;//(pulse > m) ? pulse - m : 1;
        TIM14->CR1 |= TIM_CR1_CEN;
        }
        else
        {
